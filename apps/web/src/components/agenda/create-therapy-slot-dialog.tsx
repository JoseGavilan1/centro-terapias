...
import { z } from 'zod';
import { UserRole, WEEKDAY_LABELS, Weekday } from '@centro/shared';
import { ApiError } from '@/lib/api-client';
import { useCreateTherapySlot } from '@/lib/hooks/use-therapy-slots';
...
export function CreateTherapySlotDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  const createSlot = useCreateTherapySlot();
  ...
  async function onSubmit(values: FormValues) {
    setServerError(null);
    try {
      await createSlot.mutateAsync({ ...values, validTo: values.validTo || undefined });
      ...
      if (error instanceof ApiError && error.status === 409) {
        ...
        setServerError('No se pudo crear la plantilla');
      }
    }
  }
    ...
    >
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Día, hora y profesional fijos para un paciente.</DialogDescription>
        </DialogHeader>
